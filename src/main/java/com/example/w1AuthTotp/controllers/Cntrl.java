...
public class Cntrl {
    ...
    // Use environment variables - proper Docker practice!
    private final Path privateKeyPath = Path.of(
            System.getenv().getOrDefault("PRIVATE_KEY_PATH", "/app/student_private.pem")
    );

    private final Path seedPath = Path.of(
            System.getenv().getOrDefault("DATA_DIR", "/data") + "/seed.txt"
    );
